...
def generate_and_process_with_rvc_parallel(
    ...
    # Track the next sentence index to buffer
    next_to_buffer = 0
    ...
    while (not processing_complete.is_set() or 
        ...
        try:
            ...
            # 2. Add the next sentence in sequence to the buffer if it's available
            while next_to_buffer < len(sentences):
                if next_to_buffer in completed_sentences and completed_sentences[next_to_buffer][2]:
                    _, rvc_path, _, info = completed_sentences[next_to_buffer]
                    # Add to buffer if file exists
                    if rvc_path and os.path.exists(rvc_path):
                        buffer.add(rvc_path)
                        buffered_sentences.add(next_to_buffer)
                        # Add info message in the order that sentences are added to the buffer
                        info_messages.append(info)
                        logging.info(f"Added sentence {next_to_buffer+1} to buffer")
                        next_to_buffer += 1
                    else:
                        # If file doesn't exist, skip this sentence
                        next_to_buffer += 1
                else:
                    # If the next sentence isn't ready yet, break and try again later
                    break
